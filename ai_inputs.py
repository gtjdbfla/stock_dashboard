...
MACRO_SYMBOLS = [
    ...
    ("미국 10년물 금리", "%5ETNX", "pp"),
    # 지수만으로는 "SOX가 올랐다"는 뭉뚱그린 원인만 나온다. 마이크론·엔비디아 개별
    # 주가를 더하면 "마이크론 실적발표 후 급등"처럼 당일 갭의 실제 진원지를 짚을 수 있다.
    ("마이크론(MU, 직접 경쟁사) 주가", "MU", "pct"),
    ("엔비디아(NVDA, HBM 최대 수요처) 주가", "NVDA", "pct"),
]
...
SECTOR_NEWS_QUERIES = [
    ...
    "{name} 투자의견",
    # 업황·실적 질의는 성장 쪽만 잡는다. 미중 반도체 갈등발 수출통제·관세는 별개 축이라
    # 위 질의들로는 안 걸리는데, 하루짜리 급락의 원인이 되는 경우가 있다.
    "반도체 수출 규제 관세",
]
