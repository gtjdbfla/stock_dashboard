...
`app.py`가 380KB / 함수 154개로 압도적이다(→ MAP.md). 나머지 모듈은 **전부 streamlit을 import하지 않는다** — 수집기에서도 돌아야 하므로.
...
**자동 새로고침**: 수급·과열도 16~19시 / DRAM 13·16·20시 / Capex 16시 / **AI 분석 8~16시 매시**.
...
- **모델 업그레이드로 품질을 올리는 길은 막혀 있다.** 무료 등급에서 `gemini-3.7-flash`·`gemini-pro-latest`·`gemini-3.1-pro-preview`는 목록에 뜨지만 호출하면 예외 없이 **빈 응답**. 쓸 수 있는 건 `gemini-3.6-flash`(기본)·`3.5-flash`·`3.5-flash-lite`·`flash-lite-latest`.
...
- **AI 분석 자동 생성은 브라우저 세션이 열려 있어야 돈다.** 아무도 안 열면 그 시각은 건너뛴다. 수집기로 옮기려면 과열도·DRAM 계산을 탭 렌더 밖으로 빼야 한다(위 함정). `[ai_analysis] 생성 완료 …` 로그가 쌓이므로 며칠 뒤 실제로 몇 시가 비는지 보고 판단.
- `app.py` 380KB 단일 파일. 모듈 분리 여지가 크지만 손대지 않음. 건드릴 땐 탭 렌더 의존을 먼저 확인.
