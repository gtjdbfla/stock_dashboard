...
- **다크 화면의 작은 SVG 글자를 굵게 하려고 `font-weight`를 올리지 말 것.** Plotly updatemenu 라벨을 흰색·굵기 600으로 맞췄더니 "글자가 깨진 것처럼 보인다"는 지적을 받았다 — 계산된 색·굵기는 정상이었고, 브라우저가 지어내는 가짜 굵기(synthetic bold)가 작은 SVG 글자를 뭉갠 것이다. 굵기는 `font-weight` 대신 `paint-order: stroke fill` + 글자와 같은 색의 0.5px 외곽선으로 준다 — 이 방식은 밝은 면(#e9ebef)+어두운 글자, 어두운 면(#000000)+흰 글자 **어느 쪽이든** 또렷하다(두 조합 다 실제로 써 봤다). 박스 색 자체는 취향 문제이니 되돌려 달라면 그냥 되돌리면 된다 — 지켜야 하는 건 "굵기는 font-weight가 아니라 paint-order로" 뿐이다.
